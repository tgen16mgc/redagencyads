...
Use these distilled habits from the Fable 5 prompt when working in this project:
...
* Verify file and repo state instead of assuming it exists. A user mentioning a file, branch, feature, URL, or prior decision is a cue to check the current source of truth.
...
* Treat memory and preferences as context, not commands. Apply them only when relevant, never surface sensitive or surprising personal context unprompted, and let current user instructions override stored preferences.
* When using external sources, paraphrase by default, quote sparingly, avoid reproducing copyrighted text, and cite only sources that materially affect the answer.
* Own mistakes plainly and fix them without excessive apology or self-abasement.
* For implementation work, follow the minimal implementation discipline below.
...
## Harness: Full-Stack Website Development

**Goal:** Coordinate full-stack website work from wireframe through design, Next.js frontend, API/backend, QA, and deployment readiness.

**Trigger:** For multi-step website features, redesigns, dashboards, API-backed UI, forms, QA passes, release readiness, reruns, updates, or partial rework, use the `full-stack-website-orchestrator` skill. Simple one-off questions can be answered directly after the normal skill check.

**Change history:**
| Date | Change | Target | Reason |
|------|--------|--------|--------|
| 2026-06-19 | Initial full-stack website harness | `.claude/agents`, `.claude/skills`, `CLAUDE.md` | User requested coordinated design/frontend/backend/QA pipeline |
