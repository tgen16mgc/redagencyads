...
describe("buildClientStory", () => {
  ...
  it("adds comparison deltas to KPI snapshot cards", () => {
    const previous = report({ totals: { ...totals, spend: 40_000_000, leads: 650, cpl: 61_538, ctr: 1.8 } });
    const story = buildClientStory({ report: report(), previousReport: previous, verdict, insights, compareMode: "wow", language: "en" });

    expect(story.kpis[0].delta).toEqual({ direction: "up", label: "vs previous period", value: "+20.5%" });
    expect(story.kpis[2].delta).toEqual({ direction: "up", label: "vs previous period", value: "+4.2%" });
  });

  it("builds evidence-backed cause cards instead of plain cause strings", () => {
    const story = buildClientStory({ report: report(), verdict, insights, previousReport: null, compareMode: "off", language: "en" });

    expect(story.whyItChanged[0]).toMatchObject({
      id: "verdict-loser",
      title: "Some placements are spending without proportional lead volume.",
      confidence: "high",
      evidence: "Meta attribution data is complete for the selected range.",
    });
  });
