...
export function buildClientStory({
  ...
  return {
    ...
    kpis: report.kpis.slice(0, 6).map((kpi) => ({
      key: String(kpi.key),
      label: kpi.label,
      value: formatMetric(Number(report.totals[kpi.key as keyof NormalizedRow] || 0), kpi.format, currency),
      intent: kpi.intent,
    })),
    whatChanged,
...
function buildCauses({
  ...
  return [
    verdict?.losers[0],
    verdict?.risks[0],
    insights?.rows[0]?.insight,
    report.health.checks.find((check) => check.status !== "pass")?.detail,
    fallback,
