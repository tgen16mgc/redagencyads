...
export function contextChatCopy(language: InterfaceLanguage, view: DashboardView) {
  ...
  return {
    ...
    assistantLabel: isVietnamese ? "Hỏi 9router" : "Ask 9router",
    title: isVietnamese ? "Trợ lý theo ngữ cảnh" : "Context assistant",
    ...
    working: isVietnamese ? "9router đang phân tích" : "9router is analyzing",
    unavailable: isVietnamese ? "9router chưa được cấu hình" : "9router is not configured",
    unavailableDescription: isVietnamese
      ? "Thêm NINEROUTER_KEY trên server rồi khởi động lại ứng dụng."
      : "Add NINEROUTER_KEY on the server and restart the app.",
    privacy: isVietnamese
      ? "Dữ liệu tóm tắt của tab hiện tại được gửi tới 9router. Token, file và raw payload không được gửi."
      : "A compact summary of this tab is sent to 9router. Tokens, files, and raw payloads are excluded.",
    ...
    genericError: isVietnamese ? "Không thể nhận câu trả lời từ 9router." : "Could not get an answer from 9router.",
